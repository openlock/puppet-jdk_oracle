class jdk_oracle::install (
    $version          = $jdk_oracle::version,
    $version_update   = $jdk_oracle::version_update,
    $version_build    = $jdk_oracle::version_build,
    $install_dir      = $jdk_oracle::install_dir,
    $use_cache        = $jdk_oracle::use_cache,
    $cache_source     = $jdk_oracle::cache_source,
    $platform         = $jdk_oracle::platform,
    $jce              = $jdk_oracle::jce,
    $default_java     = $jdk_oracle::default_java,
    $create_symlink   = $jdk_oracle::create_symlink,
  ...
  if $ensure == 'installed' {
    ...
    $jmxrempasswfile = "$java_home/jre/lib/management/jmxremote.password"
    $jmxremaccessfile = "$java_home/jre/lib/management/jmxremote.access"
